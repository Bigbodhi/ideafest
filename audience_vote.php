...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    $scores = json_encode($_POST['scores']);

    $total_score = 0;
    
    foreach ($_POST['scores'] as $category_scores) {
        $total_score += intval($category_scores);
    }
print_r($vote_date);
    //$stmt = $conn->prepare("SELECT * FROM audience_votes WHERE user_id = ? AND participant_id = ? AND vote_date = ?");
    // With this line
    $stmt = $conn->prepare("SELECT * FROM audience_votes WHERE user_id = ? AND participant_id = ? AND DATE(vote_date) = DATE(NOW())");
    ...
    if ($result->num_rows > 0) {
        ...
        //$stmt = $conn->prepare("INSERT INTO audience_votes (participant_id, user_id, vote_date, points ,scores) VALUES (?, ?, ?, ?, ?)");
        //$stmt->bind_param("iiiss", $participant_id, $user_id, $vote_date, $total_score, $scores);
        // With this line
        $stmt = $conn->prepare("INSERT INTO audience_votes (participant_id, user_id, points ,scores) VALUES (?, ?, ?, ?)");

        // With this line
        $stmt->bind_param("iiis", $participant_id, $user_id, $total_score, $scores);
...
<body>
    ...
    <script>
        ...
        participantSelector.addEventListener('change', async () => {
            ...
            // Load score options
            const scoreOptions = await fetch(`get_score_options.php?participant_id=${participantId}`);
